...
def _run_extract_chunk(bronze_part: Path, silver_tmp: Path, chunk_tag: str, use_lock: bool) -> int:
    cmd = [
        sys.executable,
...
def test_concurrent_writes_and_consolidation(tmp_path: Path) -> None:
    ...
    with ThreadPoolExecutor(max_workers=3) as ex:
        ...
        for fut in as_completed(futures):
            rc = fut.result()
            failures.append(rc)
    assert all(rc == 0 for rc in failures), f"At least one subprocess failed: {failures}"
...
def test_concurrent_writes_with_locks(tmp_path: Path) -> None:
    ...
    with ThreadPoolExecutor(max_workers=3) as ex:
        ...
        for fut in as_completed(futures):
            rc = fut.result()
            failures.append(rc)
    assert all(rc == 0 for rc in failures), f"Lock-protected subprocesses had failures: {failures}"
