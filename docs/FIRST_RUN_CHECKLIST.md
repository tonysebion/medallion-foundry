...
Use this checklist to validate your environment and minimize surprises on your first real run.

## 1) Environment & Dependencies
- Python: Verify version (`python --version`) matches supported range.
- Virtualenv: Activated (prompt shows `.venv`).
- Packages: Install base + dev deps:
  - `pip install -r requirements.txt`
  - `pip install -r requirements-dev.txt` (for docs/tests)
