...
- Chunked Silver writes are handled by `SilverProcessor` automatically; there is no `--stream`/`--resume` flag anymore. Rely on `_metadata.json`/`_checksums.json` plus load patterns for safety and consult `docs/framework/operations/legacy-streaming.md` for the retired CLI knobs.
...
### Chunked Silver Writes

`SilverProcessor` already slices large Bronze partitions into manageable chunks, so no separate `--stream` or `--resume` flags remain. Control chunk granularity indirectly by tuning how Bronze writes files and checking `_metadata.json`/`_checksums.json` for rerun safety. Legacy streaming options are archived in `docs/framework/operations/legacy-streaming.md`.
