...
- **Silver** promotes Bronze partitions into curated artifacts. It supports multiple Silver models (`SilverModel`) and output formats (Parquet/CSV), with `SilverProcessor` automatically chunking large Bronze partitions. Silver metadata includes runtime, model, and storage metrics to help governance decisions.
...
- Quick start & configs: `README.md` outlines how to run Bronze/Silver. `docs/framework/reference/CONFIG_REFERENCE.md` lists every configurable option (including storage metadata, `storage_scope`, Silver model/profile presets, async flags, rate limiting, tracing env vars, and resume options).
...
- Operational procedures: See `docs/framework/operations/OPERATIONS.md` for governance, testing, and sample regeneration playbooks. Error taxonomy in `docs/framework/operations/ERROR_CODES.md` aids routing; performance tuning in `docs/framework/operations/PERFORMANCE_TUNING.md` covers benchmarks and optimization.
...
- Performance: Run `scripts/benchmark.py` and consult `docs/framework/operations/PERFORMANCE_TUNING.md`.
