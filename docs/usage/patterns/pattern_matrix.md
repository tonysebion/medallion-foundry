...
1. **Event or state data?**
   ...
   - State: go to Pattern 3 (state snapshot). Choose the hybrid patterns (4–7) only when you need derived data such as per-row change events, cumulative merges, or incremental history reconstruction beyond the source snapshot.
...
- **Real-world examples:** retail catalog refreshes, nightly CRM exports, analytics-ready marketing lists that are easier to rebuild than to incrementally maintain.
...
- **Real-world examples:** payments transaction log, inventory adjustments captured as CDC, order management events where `change_type` tracks inserts/updates/deletes.
...
- **Real-world examples:** HR systems publishing HRIS current vs history tables, Salesforce snapshots of account history, or billing ledgers with effective dates.
...
- **Talk it through:** do you have reference metadata? Which columns identify the change? Are you trying to derive per-row change events (inserts/updates/deletes) rather than just replaying raw CDC?
- **Real-world examples:** audit trails that pair delta files with static customer snapshots, CRM systems that send daily delta exports plus a reference snapshot, payment feeds with both CDC and client metadata.
...
- **Talk it through:** do we reconstruct history from cumulative files? Is the goal to merge delta files on top of a reference snapshot so Silver can expose a clean SCD Type 2 timeline?
- **Real-world examples:** loyalty point ledgers that append cumulative deltas, marketing qualification states that keep cumulative logs, insurance portfolios with cumulative contract changes.
...
- **Talk it through:** which column denotes “latest”? Do you only need the freshest state and no historical output, or will those point-in-time slices feed cumulative merges later?
- **Real-world examples:** IoT telemetry snapshots showing the latest device status, pricing feeds that only publish the most recent price per SKU, status dashboards with only current values.
...
- **Talk it through:** how often do we merge, and can we rely on `delta_tag`? Are you reconstructing full history from those merges and cumulative deltas so downstream consumers can rebuild an SCD Type 2 view?
- **Real-world examples:** subscription billing snapshots with incremental merges plus cumulative change logs, data warehouses combining periodic snapshots and delta layers, risk systems that keep full transaction history while ingesting cumulative deltas.
...
Each config targets an exact Bronze sample under `sampledata/source_samples/<pattern>/…`. Every pattern folder now records daily loads spanning several weeks, so you can inspect how that pattern’s files evolve day-to-day, understand what downstream Bronze or Silver operations must handle (e.g., full rewrites vs. new CDC deltas vs. refreshed current/history rows), and know that the tests exercise the same scenarios. When you add a new pattern, copy a nearby `pattern_*.yaml`, adjust the Bronze/Silver options, regenerate fixtures, and rerun the suite so the router stays accurate.
