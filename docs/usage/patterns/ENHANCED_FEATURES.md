...
Optional OpenTelemetry spans around retries, chunk writes, and Silver chunk processing.
...
### Silver Chunking & Idempotence

`SilverProcessor` handles chunked writes automatically, so there is no longer a `--stream` or `--resume` flag to set. Trust `_metadata.json`/`_checksums.json` plus Bronze load patterns to keep reruns safe. Historical streaming knobs are documented in `docs/framework/operations/legacy-streaming.md` if you need to explain past expectations.
...
- Real-time/high-frequency: `partition_strategy: "timestamp"`
