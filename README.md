...
- Bronze hybrid samples (full reference plus CDC/incremental deltas) now under `docs/examples/data/bronze_samples/hybrid_cdc/...` and `.../hybrid_incremental/...`; rerun `python scripts/generate_sample_data.py --formats both` to rebuild them automatically.
