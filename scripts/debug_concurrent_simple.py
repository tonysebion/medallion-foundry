...
for i in range(3):
    tag=f'tag{i}'
    cmd=[sys.executable,str(ROOT/'silver_extract.py'),'--config',config,'--bronze-path',bronze,'--silver-base',silver_tmp,'--write-parquet','--artifact-writer','transactional','--chunk-tag',tag,'--use-locks','--verbose','--lock-timeout','1']
